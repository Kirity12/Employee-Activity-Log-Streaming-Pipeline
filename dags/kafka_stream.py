...
def fetch_and_load_json(url="http://10.167.161.24:5000/data"):
...
def test_kafka_connection(**context):
    ...
    try:
        admin = KafkaAdminClient(
            bootstrap_servers="broker:29092", request_timeout_ms=5000
        )
        topics = admin.list_topics()
        logging.info(f"Task {task_id}: Kafka topics found: {topics}")
        context["task_instance"].xcom_push(
            key="kafka_connection_status", value="success"
        )
        return "create_topic"
    except Exception as e:
        logging.error(f"Task {task_id}: Kafka connection failed: {e}")
        context["task_instance"].xcom_push(
            key="kafka_connection_status", value="failure"
        )
...
def process_kafka_connection_result(**context):
    ...
    kafka_connection_status = context["task_instance"].xcom_pull(
        task_ids="branch_task", key="kafka_connection_status"
    )
    task_id = context.get("task_instance").task_id
    if kafka_connection_status == "success":
        logging.info(
            f"Task {task_id}: Kafka connection was successful. Proceeding with topic creation and streaming."
        )
    elif kafka_connection_status == "failure":
        logging.error(
            f"Task {task_id}: Kafka connection failed. Aborting further steps."
        )
    else:
...
def stream_data(**context):
    producer = KafkaProducer(
        bootstrap_servers=["broker:29092"], max_block_ms=5000, api_version=(7, 4, 0)
    )
    task_id = context.get("task_instance").task_id
    curr_time = time.time()
    i = 1
        ...
        try:
            ...
            for data in res:
                ...
                try:
                    ...
                    logging.info(
                        f"Task {task_id}: Message Data {i} sent to Kafka Topic"
                    )
                except:
                    logging.error(
            ...
            logging.info(
                f"Task {task_id}: Data Production to Kafka succefully completed"
            )
        except Exception as e:
...
with DAG(
    ...
    end_success_dag = EmptyOperator(task_id="end_success_dag", dag=dag)
    end_failure_dag = EmptyOperator(task_id="end_failure_dag", dag=dag)

    create_kafka_topic = PythonOperator(
        task_id="create_topic", python_callable=create_topic
    )
    ...
    branch_task = BranchPythonOperator(
        task_id="branch_task", python_callable=test_kafka_connection, dag=dag
    )

    streaming_task = PythonOperator(
        task_id="stream_data_api", python_callable=stream_data
    )

    process_connection = PythonOperator(
        task_id="process_kafka_connection_result",
        python_callable=process_kafka_connection_result,
    )  # Fixed task id here
    ...
    streaming_task >> end_success_dag
    slack_alert_task >> end_failure_dag
